...
\subsection{A summary of the situation}
...
\bigskip
The Democrats have a lot of work to do in order to gain the Senate. They need 2 extra seats. 26 democrat seats are up for election. So they have to hold onto those AND gain an extra 2 seats to get to 28 in this election. 
\begin{itemize}
...
\subsection{A detailed approach}
\newthought{The micro view: the State level}.
The Senate seems to be the hardest to win for the Democrats. Nate Silver suggested a more detailed method to calculate the probabilities of a win (or loss) \emph{on the State level}. The result of a simulation (N=10.000) based on the interpretation by Nate Silver of the probabilities of ''Safe"=0.95, ''Likely"=0.85, ''Lean"=0.75, ''Tilt"=0.60 (not included in the article) and ''Toss up"=0.50 is (Figure~\ref{fig:senatesim}) is that the Democrats are likely to \emph{lose} 3 seats. Gaining 2 seems very unlikely!
...
<<>>=
print(paste("Probability of 28 or more seats = ",
            round(prob,2), "%"))
@
...
\newthought{The macro view: taking correlation into account.}
However: these calculations are based on the assumption that all these races are \emph{independent}. In reality this is not the case, and the outcomes are \emph{correlated}. The correlation can go from 0 (independent = Figure~\ref{fig:senatesim}) to 1. The value of 1 is the situation in which when you win one ''toss-up", you win them all\sidenote{I assume that this tweeking is performed on all races where at least one of the raters uses the term ''Toss-up"}. When you tweek the simulation for maximum correlation, you get Figure~\ref{fig:senatesimmax}.
<<echo=FALSE>>=
# Simulation with maximum correlation
race.max <- race
for (i in 1:N) {
  if(sum(race.max[i,6:14] >= 1)) {race.max[i,6:14] <- 1}
}
race.result.max <- rowSums(race.max)
prob.max <- sum(race.result.max > 27)/N
@

<<>>=
print(paste("Probability of 28 or more seats = ",
            round(100*prob.max,2), "%"))
@

<<label=senatesimmax,fig=TRUE,include=FALSE, echo=FALSE>>=
ggplot() +
  aes(race.result.max) +
  geom_histogram(binwidth=1, colour="black", fill="white") +
  labs(title="Simulation with maximum correlation", subtitle="About 50% chance of winning the Senate", x="Number of Democratic seats") +
  scale_x_discrete(limits=min(race.result.max):max(race.result.max), labels = c(min(race.result.max):max(race.result.max))) +
  JT.theme
@

\begin{marginfigure}
\includegraphics[width=1\textwidth]{USC-senatesimmax}
\caption{Histogram of simulation with maximul correlation}
\label{fig:senatesimmax}
\setfloatalignment{b}% forces caption to be bottom-aligned
\end{marginfigure}

The probability that the Democrats win the Senate rises to \Sexpr{round(100*prob.max,2)} \%\sidenote{Nate Silvers has it on 50\%}. The truth will be somewhere between the two extremes: \Sexpr{round(100*prob,2)} and \Sexpr{round(100*prob.max,2)}.

We can adjust the correlation. Nate Silver suggests a rule of thumb: 50\% is local and 50\% depends on supra-national factors (the correlation). 

<<echo=FALSE>>=
# Simulation with 50% correlation
race.part <- race
for (i in 1:N) {
  if(sum(race.part[i,6:14] >= 1)) {
    for (j in 6:14) {
      if(race.part[i,j] == 0) {race.part[i,j] <- 0.5}
    }
  }
}
race.result.part <- rowSums(race.part)
prob.part <- sum(race.result.part > 27)/N
@

<<>>=
print(paste("Probability of 28 or more seats = ",
            round(100*prob.part,2), "%"))
@

<<label=senatesimpart,fig=TRUE,include=FALSE, echo=FALSE>>=
ggplot() +
  aes(race.result.part) +
  geom_histogram(binwidth=1, colour="black", fill="white") +
  labs(title="Simulation with 50% correlation", x="Number of Democratic seats") +
  scale_x_discrete(limits=min(race.result.part):max(race.result.part), labels = c(min(race.result.part):max(race.result.part))) +
  JT.theme
@

\begin{marginfigure}
\includegraphics[width=1\textwidth]{USC-senatesimpart}
\caption{Histogram of simulation with 50\% correlation}
\label{fig:senatesimpart}
\setfloatalignment{b}% forces caption to be bottom-aligned
\end{marginfigure}

Another approach is to include the supra-national effects into the probabilities that we assign to the labels ''Safe", ''Likely", etc.
...
\subsection{Simulaton of the House election}

The New York Times gave on Feb. 7th 2018 a list of all the districts and their status as being ''Solid Dem", ''Lean Dem" etc. Using the same probabilities for a democratic win as we used in the simulation for the Senate we get the following summary:
...
Let's do a simulation of 10,000 elections:
<<echo=FALSE>>=
# Simulation
# Number of simulations
N <- 10000
...
<<>>=
print(paste("Probability of 218 or more seats = ", round(prob.House,2), "%"))
@
...
ggplot() +
  ...
  labs(title="House simulation", x="Number of Democratic seats", caption="Based on ratings as published in the New York Times, 2018-02-07") +
  scale_x_discrete(limits=seq(min(race.result.House), max(race.result.House), by=5), 
                   labels= seq(min(race.result.House),max(race.result.House), by=5)) +
  JT.theme
@
