...
\bigskip
The Democrats need 2 extra seats. 26 democrat seats are up for election. So they have to hold onto those AND gain an extra 2 seats to get to 28 in this election. 
\begin{itemize}
...
ggplot() +
  ...
  labs(title="23 seats = loss of 3!", subtitle="Winning 28 seats is very unlikely", x="Number of Democratic seats") +
...
\caption{Histogram of simulation}
\label{fig:senatesim}
\setfloatalignment{b}% forces caption to be bottom-aligned
\end{marginfigure}

From this simulations we can see that the Democrats will (only) win \Sexpr{round(mean(race.result),0)} seats, which is a loss of \Sexpr{26 - round(mean(race.result),0)} seats! The probability of winning at least 28 seats is \Sexpr{round(prob,2)} \%.

\subsection{The macro view: taking correlation into account}
Nate Silver argues that calculating state by state assumes that each state is run in a ''vacuum", completely independent of all others. However, the races are interconnected by factors that supersede the state level. That such factors exist is shown by the fact that historically the net number of seats won (positive) or lost (negative) is highly correlated with the approval rate of the incumbent president. This is clearly a factor that is above the state level and influences the total number of seats (not a particular one).

<<label=midterm_senate1,fig=TRUE,include=FALSE, echo=FALSE>>=
midterm <- read.csv("data/midterm_results.csv",header=TRUE, sep=";",col.names=c("Year","President","Party", "Approval","Disapproval","Gain.Senate","Gain.House", "Term","GenDem","GenRep","PresParty"), na.strings="NA", stringsAsFactors = FALSE)
...
ggplot(data=midterm) +
  ...
  geom_line(aes(x, y, linetype = mean.loss), meanloss.senate, colour="red") +
...
We can fit a linear model through all these points. The $R^{2}$-value is \Sexpr{round(summary(modall.senate)$r.squared,2)}. That is not very impressive.
...
@
\medskip
The $R^{2}$-value is \Sexpr{round(summary(modterm1)$r.squared,2)}. That is better, but not great.
...
ggplot(midterm1, aes(x=Approval, y=Gain.Senate)) + 
  ...
  geom_line(aes(x, y, linetype = mean.loss), meanloss.senate, colour="red") +
  labs(title="Gain (+) or Loss (-) of seats in the Senate midterm elections\nin function of approval rate (%) for  first term president") +
  JT.theme
@
...
The confidence interval is -5 to -19 seats. This correlation indicates that instead of a gain of 3 seats by the Republicans, the could end up with a net loss of 11!
\newpage
...
\medskip
<<echo=FALSE>>=
midterm %>% select(Gain.Senate, Disapproval, Term) %>% filter(Term==1) -> midterm1dis
modterm1dis <- lm(Gain.Senate ~ Disapproval, data=midterm1dis)
@
\medskip
...
modterm1diff <- lm(Gain.Senate ~ Diff, data=midterm1)
@
\medskip
...
modterm1ratio <- lm(Gain.Senate ~ Ratio, data=midterm1)
@
\medskip
The $R^{2}$-value for the model based on the \emph{Ratio} is \Sexpr{round(summary(modterm1ratio)$r.squared,2)}. No better.

\medskip

\newthought{None of the models are very good} because the $R^{2}$-value is low. This means that there is some effect of the presidents approval rate on the net result in the Senate, but it should be treated with care.

\subsection{Connecting the micro and the macro}

<<echo=FALSE>>=
Senate$DemProbCor <- Senate$DemProb + (59/40 - 1)*(1-Senate$DemProb)
# Simulation
# Number of simulations
N <- 10000
race <- matrix(0, N, nrow(Senate))
set.seed(2018)
for (i in 1:nrow(Senate)) {
    race[,i] <- rbinom(N, 1, Senate$DemProbCor[i])
}
race.result <- rowSums(race)
prob <- sum(race.result > 27)/N
@

The easiest way is to link the macro (the approval rate of the president) with the micro (the likelihood of a win by the Democratic candidate in a specific state election), is to take the probability on the micro level, and increase it. However, as the correlation is weak, this increase should be modest. The model predicts that an approval rate of 59\% would result in a zero net gain. An approval rate of 40\% leads to a net loss of about 11 seats. A race which at the moment is classified as ''Lean Dem" (probability of a win = 0.75) gets an upgrade to $0.75 + (1-0.75)*(\frac{59}{40}-1)=0.87$. The results of a 100\% upgrade are in Figure~\ref{fig:senatesimfull}. With this extreme upgrade we can get to 28 seats and more. The calculated probability of the Democrats winning the Senate in this case is \Sexpr{round(100*prob,2)}\%.

<<label=senatesimfull,fig=TRUE,include=FALSE, echo=FALSE>>=
ggplot() +
  aes(race.result) +
  geom_histogram(binwidth=1, colour="black", fill="white") +
  labs(title="Full upgrade of probabilities of democratic win", subtitle="28 seats or more is possible", x="Number of Democratic seats") +
  scale_x_discrete(limits=min(race.result):max(race.result), labels = c(min(race.result):max(race.result))) +
  JT.theme
@

\begin{marginfigure}[-7cm]
\includegraphics[width=1\textwidth]{USC-senatesimfull}
\caption{Histogram of simulation}
\label{fig:senatesimfull}
\setfloatalignment{b}% forces caption to be bottom-aligned
\end{marginfigure}


<<echo=FALSE>>=
# p is the percentage of the upgrade of the probabilities
p <- 0.5
Senate$DemProbCor <- Senate$DemProb + p*(59/40 - 1)*(1-Senate$DemProb)
# Simulation
# Number of simulations
N <- 10000
race <- matrix(0, N, nrow(Senate))
...
<<label=senatesim50,fig=TRUE,include=FALSE, echo=FALSE>>=
ggplot() +
  aes(race.result) +
  geom_histogram(binwidth=1, colour="black", fill="white") +
  labs(title="50% upgrade of probabilities of democratic win", subtitle="26 seats (status quo) seems likely", x="Number of Democratic seats") +
  scale_x_discrete(limits=min(race.result):max(race.result), labels = c(min(race.result):max(race.result))) +
  JT.theme
@

\begin{marginfigure}[-1cm]
\includegraphics[width=1\textwidth]{USC-senatesim50}
\caption{Histogram of simulation}
\label{fig:senatesim50}
\setfloatalignment{b}% forces caption to be bottom-aligned
\end{marginfigure}

But the model should be treated with caution. The results of a 50\% upgrade are in Figure~\ref{fig:senatesim50}. The calculated probability of the Democrats winning the Senate in this case is \Sexpr{round(100*prob,2)}\%. With this modest adjustment of the probabilities of a Democratic win, the end result is a status quo. The Democrats do not win the Senate, but the status quo means that the Republicans do not get past a 60 seat majority.

\newthought{Conclusion} I do not see any reason that the probability for a Democratic win is higher than \textbf{20\%}.
...
ggplot() +
  ...
  scale_x_continuous(breaks=seq(200, 400, by=5), 
                     labels= seq(200,400, by=5)) +
  JT.theme
@
...
Starting from the data per state, the Democrats are likely to \emph{win} 17 seats, getting them to 210. Close to 218 in order to win the House. The probability to do so is \emph{about 5\%}.

\subsection{The macro view}

\newthought{There is a strong correlation} between the number of seats \emph{gained} or \emph{lost} in a midterm election and the approval rate of the president\sidenote[][-2cm]{https://arxiv.org/pdf/1503.07545.pdf}. I have redone the calculation, including the 2014 result, a correction of the 1950 approval rating of Truman, and adding the disapproval rating and the term of the election.
...
\medskip
As in the case of the Senate a model based only on \emph{first term}-presidents is better.
\medskip
<<echo=FALSE>>=
midterm %>% filter(Term==1) -> midterm1
...
<<echo=FALSE>>=
House$DemProbCor <- House$DemProb + (65/40 - 1)*(1-House$DemProb)
N <- 10000
race.House <- matrix(0, N, sum(House$Number))
set.seed(2018)
for (i in 1:N) {
...
I use the same method to link the macro (the approval rate of the president) with the micro (the likelihood of a win by the Democratic candidate in a specific state election) by increasing the probability on the micro level. The correlation is much stronger here. The model predicts that an approval rate of 65\% would result in a zero net gain. A race which at the moment is classified as ''Lean Dem" (probability of a win = 0.75) gets an upgrade to $0.75 + (1-0.75)*(\frac{65}{40}-1)=0.90$. The results of a 100\% upgrade are in Figure~\ref{fig:housesimfull}. With this extreme upgrade we can get to 28 seats and more. The calculated probability of the Democrats winning the House in this case is \Sexpr{round(100*prob.House,2)}\%.
...
ggplot() +
  ...
  labs(title="House: full upgrade of probabilities of democratic win", 
       subtitle="350 seats = gain of 157 is impossible", 
       x="Number of Democratic seats") +
  scale_x_continuous(breaks = seq(300, 400, by=5),
                     labels = seq(300, 400, by=5)) +
  JT.theme
@
...
# p is the percentage of the upgrade of the probabilities
p <- 0.1
House$DemProbCor <- House$DemProb + p*(65/40 - 1)*(1-House$DemProb)
N <- 10000
race.House <- matrix(0, N, sum(House$Number))
set.seed(2018)
for (i in 1:N) {
...
<<label=housesim10,fig=TRUE,include=FALSE, echo=FALSE>>=
ggplot() +
  aes(race.result.House) +
  geom_histogram(binwidth=1, colour="black", fill="white") +
  labs(title="House: 10% upgrade of probabilities of democratic win",
       subtitle="224 seats = gain of 31: seems likely!",
       x="Number of Democratic seats") +
  scale_x_continuous(breaks = seq(200, 250, by=5),
                   labels = seq(200, 250, by=5)) +
  JT.theme
@

\begin{marginfigure}[0cm]
\includegraphics[width=1\textwidth]{USC-housesim10}
\caption{Histogram of simulation}
\label{fig:housesim10}
\setfloatalignment{b}% forces caption to be bottom-aligned
\end{marginfigure}

The results of a 10\% upgrade are in Figure~\ref{fig:housesim30}. The calculated probability of the Democrats winning the House in this case is \Sexpr{round(100*prob.House,2)}\%. 

\newthought{Conclusion}. Winning ths House seems much more likely! Only a small ''macro" push will give the Democrats a majority in the House. Let's keep it at 70\% for the moment.
...
\label{subsec:limits}
Let's take a short theoretical detour, that is not fixed on the problem of the US-elections. We consider two events ($A$ and $B$). The alternative for event $A$ are its complement $A^{C}$. These two events, $A$ and $A^{C}$ are mutually exclusive. This means that if $A$ occurs, $A^{C}$ cannot occur. Idem for $B$ and $B^{C}$. If the probability that $A$ occurs is $P[A]$ than it follows that the probability for $A^{C}$ is equal to $1-P[A]$. Combining the two events $A$ and $B$ and their complements we get the Tabel~\ref{tab:ABstart}.
...
\section{2018-02-05}
\newthought{People have not changed there mind since 2016}.

<<label=leadCvsdisapproval,fig=TRUE,include=FALSE, echo=FALSE>>=
ggplot(data=Senate) +
  geom_point(aes(x=LeadClinton16, y=Disapprove, color=Party)) +
  labs(title="Strong correlation between the percentage\ndifference in 2016 and dissaproval of Trump", x="Lead of Clinton vs. Trump in 2016", y="Disapproval rate of Trump", caption="https://en.wikipedia.org/wiki/United_States_presidential_election,_2016") +
  JT.theme
@

\begin{marginfigure}[0cm]
\includegraphics[width=1\textwidth]{USC-leadCvsdisapproval}
\caption{}
\label{fig:leadCvsdisapproval}
\setfloatalignment{b}% forces caption to be bottom-aligned
\end{marginfigure}
