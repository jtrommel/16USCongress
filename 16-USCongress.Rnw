...
<<echo=FALSE>>=
JT.ProbCalc <- function(JT.df) {
  colnames(JT.df) <- c("Party","Rater")
  prob <- numeric(nrow(JT.df))
  for (i in 1:nrow(JT.df)) {
    if (grepl("Safe", JT.df$Rater[i])) {prob[i] <- 0.95}
    if (grepl("Like", JT.df$Rater[i])) {prob[i] <- 0.85}
    if (grepl("Lean", JT.df$Rater[i])) {prob[i] <- 0.75}
    if (grepl("Tilt", JT.df$Rater[i])) {prob[i] <- 0.60}
    if (grepl("Toss", JT.df$Rater[i])) {prob[i] <- 0.5}
...
The Senate seems to be the hardest to win for the Democrats. Nate Silver suggested a more detailed method to calculate the probabilities of a win (or loss). The result of a simulation based on the interpretation by Nate Silver of the probabilities of ''Safe"=0.95, ''Likely"=0.85, ''Lean"=0.75, ''Tilt"=0.60 (not included in the article) and ''Toss up"=0.50 is (Figure~\ref{fig:senatesim}) is that the Democrats are likely to \emph{lose} 3 seats. Gaining 2 seems very unlikely!
...
Senate$DemProb <- (JT.ProbCalc(JT.Cook) + JT.ProbCalc(JT.InsideElections) + JT.ProbCalc(JT.Sabato))/3
# Simulation
...
ggplot() +
  ...
  labs(title="23 seats = loss of 3!", subtitle="Winning 28 seats is very unlikely") +
...
However: these calculations are based on the assumption that all these races are \emph{indepent}. In reality this is not the case, and the outcomes are \emph{correlated}. The correlation can go from 0 (independent = Figure~\ref{fig:senatesim}) to 1. The value of 1 is the situation in which when you win one ''toss-up", you win them all\sidenote{I assume that this tweeking is performed on all races where at least one of the raters uses the term ''Toss-up"}. When you tweek the simulation for maximum correlation, you get Figure~\ref{fig:senatesimmax}.
<<echo=FALSE>>=
...
ggplot() +
  aes(race.result.max) +
  geom_histogram(binwidth=1, colour="black", fill="white") +
  labs(title="Simulation with maximum correlation", subtitle="About 50% chance of winning the Senate") +
