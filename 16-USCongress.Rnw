...
midterm <- read.csv("data/midterm_results.csv",header=TRUE, sep=";",col.names=c("Year","President","Party", "Approval","Disapproval","Gain.Senate","Gain.House", "Term","GenDem","GenRep","PresParty","Seats.up.ratio"), na.strings="NA", stringsAsFactors = FALSE)
...
The easiest way is to link the macro (the approval rate of the president) with the micro (the likelihood of a win by the Democratic candidate in a specific state election), is to take the probability on the micro level, and increase it \emph{when the approval rate is below 60}. The limit of 60 comes from Figure~\ref{fig:midterm1_senate_lm}. It is the approval rate where the presidents party does not lose any senate seats. The degree of the upgrade will then depend on the difference between the actual approval rate and the limit of 60. At the moment the scores are:
...
A possible formula could be that 

\begin{enumerate} 
  \item if the actual approval rating is between 50 and 60 we upgrade the probability of a democratic win with \emph{half} a level: a ''Lean D" (prob=0.75) becomes something between a ''Lean D" (prob=0.75) and a ''Likely D" (prob=0.85) and gets a probability of 0.80. It boils down to a 5\% upgrade.
  \item if the actual approval rating is between 40 and 50 we upgrade the probability of a democratic win with a \emph{full} level: a ''Lean D" (prob=0.75) becomes a ''Likely D" (prob=0.85) and gets a probability of 0.85. It comes down to a 10\% upgrade.
\end{enumerate}

In this case the approval rating is 40\% so we apply the upgrade with one level and the results are given by Figure~\ref{fig:senatesimfull}.

<<echo=FALSE>>=
Senate$DemProbCor <- ifelse(((JT.ProbCalc(JT.Cook, scores) + 
                                 JT.ProbCalc(JT.InsideElections, scores) + 
                                 JT.ProbCalc(JT.Sabato, scores))/3 + 0.1)>1, 
                             0.999, 
                             ((JT.ProbCalc(JT.Cook, scores) + 
                                 JT.ProbCalc(JT.InsideElections, scores) + 
                                 JT.ProbCalc(JT.Sabato, scores))/3 + 0.1))
# Simulation
# Number of simulations
N <- 10000
race <- matrix(0, N, nrow(Senate))
...
ggplot() +
  aes(race.result) +
  geom_histogram(binwidth=1, colour="black", fill="white") +
  labs(title="Senate: upgrade of probabilities of democratic win", subtitle="26 seats or status quo", x="Number of Democratic seats") +
  scale_x_discrete(limits=min(race.result):max(race.result), labels = c(min(race.result):max(race.result))) +
  JT.theme
@
...
With this  adjustment of the probabilities of a Democratic win, the end result is a status quo which means that the Republicans do not get past a 60 seat majority.. The calculated probability of the Democrats winning the Senate \emph{with the upgrade} is \Sexpr{round(100*prob,2)}\%.

\newthought{Conclusion} I do not see any reason that the probability for \emph{a Democratic win of the Senate} is higher than \textbf{20\%}.
...
I use the same method to link the macro (the approval rate of the president) with the micro (the likelihood of a win by the Democratic candidate in a specific state election) by increasing the probability on the micro level.
...
The results of a 10\% upgrade are in Figure~\ref{fig:housesimfull}. The calculated probability of the Democrats winning the House in this case is \Sexpr{round(100*prob.House,2)}\%. 

\newthought{Conclusion}. The result of the uprade is the same as the prediction of the number of seats won by the model based on the approval rating. Winning ths House seems much more likely! Only a small ''macro" push will give the Democrats a majority in the House. Let's keep it at 70\% for the moment.
