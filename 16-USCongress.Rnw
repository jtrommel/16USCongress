...
\newthought{What are the conclusions about the matrix}? First: the starting data are crucial. Usually we start with the probabilities of a win for the Democrats in the Senate and in the House as a given. We take these probabilities from worthies like Nate Silver, or our own calculations. Then we try to find the probabilities of the combinations. However: that is not necessary the best approach. You absolutely need one of the probabilities (Senate of House) to start with. But once you have chosen that one (for example: probability of a win of the House for the Democrats = $P[H_{Dem}]=70\%$), you need two other choices. One of these could be the probability that the Democrats win the Senate = $P[S_{Dem}]=20\%$, but that is not necessary. You can also start with two of the conditional probabilities, or one conditional probability and a Likelihood Ratio etc. At the moment this leads me to my forecast of \ref{sec:20180217}.
