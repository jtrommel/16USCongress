...
The Senate seems to be the hardest to win for the Democrats. Nate Silver suggested a more detailed method to calculate the probabilities of a win (or loss). Let's put his data in a dataframe.
...
Senate$DemProb <- (JT.ProbCalc(Senate$Cook) + JT.ProbCalc(Senate$InsideElections) + JT.ProbCalc(Senate$Sabato))/3
for (i in (1:nrow(Senate))) {
  if (Senate$Party[i] == "R") {Senate$DemProb[i] <- 1 - Senate$DemProb[i]}
}
@
