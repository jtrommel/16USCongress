...
ggplot() +
  aes(race.senate.result) +
  geom_histogram(binwidth=1, colour="black", fill="white") +
  labs(title="Senate: simulation on the micro level", subtitle="Small shift to 22 seats = loss of 2! Winning 28 seats is very unlikely", x="Number of Democratic seats") +
  scale_x_discrete(limits=min(race.senate.result):max(race.senate.result), labels = c(min(race.senate.result):max(race.senate.result))) +
  JT.theme
@
