...
\label{tab:Forecast3a}
\end{margintable}

We can see that $a > b$ and $(a+b)>1$. From \ref{subsec:limits} it follows that only two completely dependent solutions are mathematically possible: $P[H_{Dem}|S_{Dem}]=1$ and $P[H_{Rep}|S_{Dem}]=1$. These assumptions lead to one ''upward" table (Table~\ref{tab:Forecast3b}) and one ''downward" table (Table~\ref{tab:Forecast3c}).
...
\label{tab:Forecast3c}
\end{margintable}

Assuming an \emph{upward partial dependency} between $S_{Dem}$ and $H_{Dem}$ of 30\%, the value in the top left corner will be $0.225 + 0.3*(0.375 - 0.225)=0.27$. This gives us Tabel~\ref{tab:Forecast3d}.
